...
def cal_qed(gen):
    ...
    for i in range(len(gen)):
        try:
            # print(f"Processing molecule {i+1}/{len(gen)}: {gen[i]}")
            mol = Chem.MolFromSmiles(gen[i])
            if mol is not None:
                total_qed += QED(mol)
                valid_count += 1
            else:
                print(f"Invalid SMILES: {gen[i]}, skipping.")
        except Exception as e:
            print(f"Error processing molecule {gen[i]}: {e}")
            continue
...
def get_all_metrics(gen, n_jobs=1,
    ...
    # with open(args.valid_gen_path, mode='w', newline='') as file:
    #     writer = csv.writer(file)
        
    #     # 将每个元素作为一行写入CSV
    #     for item in gen_valid:
    ...
    if len(gen_valid) != 0:
        ...
        metrics['qed']=cal_qed(gen_valid)
        metrics['SA']=cal_SA(gen_valid)
...
def main(args):
    ...
    if args.train_path is not None:
        train = pd.read_csv(args.train_path, sep=' ')["SMILES"].tolist()
    ...
    for i in range(10):
        ...
        for key, value in metrics.items():
            ...
            #     Internal_diversity2.append(value)
                ...
                qed_score.append(value)
            elif key == "SA":
                SA_score.append(value)
            # elif key == "Filter":
            #     Filter.append(value)
    ...
    metrics_all["qed"] = sum(qed_score) / len(qed_score)
    metrics_all["SA"] = sum(SA_score) / len(SA_score)
...
def get_parser():
    ...
    parser.add_argument('--ref_smiles', default="/home/lianhy/ScaffoldGVAE-master/data/parp1/parp1_candidates.smi", type=str,
    ...
    parser.add_argument('--gen_path',default='/home/lianhy/ScaffoldGVAE-master/data/parp1/parp1_sample_5k.csv',type=str, required=False,help='Path to generated molecules csv')
    parser.add_argument('--output',default='/home/lianhy/ScaffoldGVAE-master/data/parp1/parp1_metrics_5k.csv',type=str, required=False,help='Path to save results csv')
